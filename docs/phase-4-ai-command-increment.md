...
Natural-language generation is available only to signed-in users on a server configured with an OpenAI API key; guests retain the local structured editor. The command dialog visibly enforces the 2,000-character prompt limit; the server independently caps the combined serialized prompt/context request at 64 KiB before the provider is called. Broader modeling operations, streaming chat, response signatures, quotas, and multi-level AI redo remain for later increments. AI undo is stack-safe and persisted, but currently restores only the latest still-applied AI command.

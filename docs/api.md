...
Phase 1 routes: `POST /v1/auth/register`, `/login`, `/refresh`, `/logout`; `GET /v1/users/me`; CRUD `/v1/projects`; `POST /v1/projects/:id/sync`; `GET /v1/projects/:id/changes`; and `GET /health`. Phase 4 adds authenticated `POST /v1/ai/commands`, which returns a schema-constrained CAD command and token usage; the OpenAI API key remains server-only. Phase 8 adds super-admin-only `GET /v1/admin/overview`, `/users`, `/projects`, and `/audit`; the audit feed contains only AI-usage and sync-mutation metadata, never prompts, payloads, manifests, or credentials.
